...
function addValues(val1, val2) {
    if (isNaN(val1) || isNaN(val2)) {
        addToPage("One or both of the inputs is not a number and they equal :" + val1 + " " + val2 + ".");
    }
    else {
        addToPage("Both parameters are numbers. They equal: " + (val1 + val2));
    }
}
...
function scopeExample() {
    let localVar = "This is a local variable inside the scopeExample() function";
    addToPage(localVar);
}
...
scoreBasket();
scoreThree();
addToPage("The score after 3 baskets and a three-pointer is: " + score + " and the score with another 3 pointer is: " + scoreThree());
...
addToPage("The number variable after first call: " + number);
